use wasmi::{
  Externals, RuntimeArgs, Trap, TrapCode,
  RuntimeValue,
};
  ...
  fn invoke_index(
    ...
    match HostFunctions::from(index) {
		HostFunctions::ReadRegister => {
			let register_id: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			let ptr: u64 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::WriteRegister => {
			let register_id: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			let data_len: u64 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
			let data_ptr: u64 = args.nth_checked(2).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::SignerAccountPublicKey => {
			let register_id: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			self.signer_account_pk(register_id)
				.map(|_| None )
				.map_err(vmlogicerr_to_trap)
		},
		...
		HostFunctions::AccountBalance => {
			let balance_ptr: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::Sha256 => {
			let value_len: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			let value_ptr: u64 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
			let register_id: u64 = args.nth_checked(2).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::Ecrecover => {
			let hash_len: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			let hash_ptr: u64 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
			let sign_len: u64 = args.nth_checked(2).map_err(|_| TrapCode::UnexpectedSignature)?;
			let sig_ptr: u64 = args.nth_checked(3).map_err(|_| TrapCode::UnexpectedSignature)?;
			let v: u64 = args.nth_checked(4).map_err(|_| TrapCode::UnexpectedSignature)?;
			let malleability_flag: u64 = args.nth_checked(5).map_err(|_| TrapCode::UnexpectedSignature)?;
			let register_id: u64 = args.nth_checked(6).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::PanicUtf8 => {
			let len: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			let ptr: u64 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::Abort => {
			let msg_ptr: u32 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			let filename_ptr: u32 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
			let line: u32 = args.nth_checked(2).map_err(|_| TrapCode::UnexpectedSignature)?;
			let col: u32 = args.nth_checked(3).map_err(|_| TrapCode::UnexpectedSignature)?;
			self.abort(msg_ptr, filename_ptr, line, col)
				.map(|_| None )
				.map_err(vmlogicerr_to_trap)
		},
		HostFunctions::PromiseCreate => {
			let account_id_len: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			let account_id_ptr: u64 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
			let method_name_len: u64 = args.nth_checked(2).map_err(|_| TrapCode::UnexpectedSignature)?;
			let method_name_ptr: u64 = args.nth_checked(3).map_err(|_| TrapCode::UnexpectedSignature)?;
			let arguments_len: u64 = args.nth_checked(4).map_err(|_| TrapCode::UnexpectedSignature)?;
			let arguments_ptr: u64 = args.nth_checked(5).map_err(|_| TrapCode::UnexpectedSignature)?;
			let amount_ptr: u64 = args.nth_checked(6).map_err(|_| TrapCode::UnexpectedSignature)?;
			let gas: u64 = args.nth_checked(7).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::PromiseThen => {
			...
			let account_id_len: u64 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
			let account_id_ptr: u64 = args.nth_checked(2).map_err(|_| TrapCode::UnexpectedSignature)?;
			let method_name_len: u64 = args.nth_checked(3).map_err(|_| TrapCode::UnexpectedSignature)?;
			let method_name_ptr: u64 = args.nth_checked(4).map_err(|_| TrapCode::UnexpectedSignature)?;
			let arguments_len: u64 = args.nth_checked(5).map_err(|_| TrapCode::UnexpectedSignature)?;
			let arguments_ptr: u64 = args.nth_checked(6).map_err(|_| TrapCode::UnexpectedSignature)?;
			let amount_ptr: u64 = args.nth_checked(7).map_err(|_| TrapCode::UnexpectedSignature)?;
			let gas: u64 = args.nth_checked(8).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::PromiseAnd => {
			let promise_idx_ptr: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			let promise_idx_count: u64 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::PromiseBatchActionDeployContract => {
			...
			let code_ptr: u64 = args.nth_checked(2).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::PromiseBatchActionFunctionCall => {
			let promise_index: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			let method_name_len: u64 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
			let method_name_ptr: u64 = args.nth_checked(2).map_err(|_| TrapCode::UnexpectedSignature)?;
			let arguments_len: u64 = args.nth_checked(3).map_err(|_| TrapCode::UnexpectedSignature)?;
			let arguments_ptr: u64 = args.nth_checked(4).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::PromiseResult => {
			let result_idx: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			let register_id: u64 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
			self.promise_result(result_idx, register_id)
				.map(|ret| Some(ret.into()) )
				.map_err(vmlogicerr_to_trap)
		},
		HostFunctions::PromiseReturn => {
			let promise_idx: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			self.promise_return(promise_idx)
				.map(|_| None )
				.map_err(vmlogicerr_to_trap)
		},
		HostFunctions::StorageWrite => {
			let key_len: u64 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			let key_ptr: u64 = args.nth_checked(1).map_err(|_| TrapCode::UnexpectedSignature)?;
			let value_len: u64 = args.nth_checked(2).map_err(|_| TrapCode::UnexpectedSignature)?;
			let value_ptr: u64 = args.nth_checked(3).map_err(|_| TrapCode::UnexpectedSignature)?;
			let register_id: u64 = args.nth_checked(4).map_err(|_| TrapCode::UnexpectedSignature)?;
		...
		HostFunctions::Gas => {
			let gas_amount: u32 = args.nth_checked(0).map_err(|_| TrapCode::UnexpectedSignature)?;
			self.gas(gas_amount)
				.map(|_| None )
				.map_err(vmlogicerr_to_trap)
		},
