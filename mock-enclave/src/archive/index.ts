...
import { maybeInitialzeLocalDatabase } from './host/db';
...
const main = async () => {
  ...
  await maybeInitialzeLocalDatabase(db);

  // 3. fetch the current shard info
  await indexer.fetchShardInfo(chain, shard.getAddress());
  ...
  while (lightSwitch) {
      ...
      blockNumber < localMetadata.high_local_block; blockNumber ++ ) 
    {
      try {
        ...
        if (
          !block || (!block.calls || block.calls.length === 0)
        ) {
        ...
        if (Dispatcher.isDispatchable(db, blockNumber)) {
          ...
          if (Object.keys(callsOfBlock).length > 0) {
            // const [dbOps, newStateRoot] = await Dispatcher.dispatchBatch(
            //   callsOfBlock, 
