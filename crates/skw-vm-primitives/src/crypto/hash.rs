use crate::crypto::util::{Packable, Point, Scalar};
...
macro_rules! hash_chain {
    ($h:expr, $d:expr $(, $dd:expr)*) => {
        hash_chain!($crate::crypto::hash::_hash_chain($h, $d) $(, $dd)*)
    };
    ($h:expr) => {
        $h
    };
...
macro_rules! hash {
    ($($d:expr),*) => {
        $crate::crypto::hash::_hash_result(hash_chain!($crate::crypto::hash::_hash_new::<$crate::crypto::hash::Hash256>() $(, $d)*))
    };
}

macro_rules! hash_s {
    ($($d:expr),*) => {
        $crate::crypto::hash::_hash_to_scalar(hash!($($d),*))
    };
}
...
macro_rules! prs {
    ($($d:expr),*) => {
        $crate::crypto::hash::_prs_result(hash_chain!($crate::crypto::hash::_hash_new::<$crate::crypto::hash::Hash512>() $(, $d)*))
    };
}
