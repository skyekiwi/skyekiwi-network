...
impl DBKey {
    pub fn len(&self) -> usize {
        match self {
            DBKey::Account { account_id } => col::ACCOUNT.len() + account_id.len(),
            DBKey::ContractCode { account_id } => col::CONTRACT_CODE.len() + account_id.len(),
            DBKey::AccessKey { account_id, public_key } => {
                col::ACCESS_KEY.len() * 2 + account_id.len() + public_key.len()
            }
            DBKey::ReceivedData { receiver_id, data_id } => {
                col::RECEIVED_DATA.len()
                    + receiver_id.len()
                    + ACCOUNT_DATA_SEPARATOR.len()
                    + data_id.as_ref().len()
            }
            DBKey::PostponedReceiptId { receiver_id, data_id } => {
                col::POSTPONED_RECEIPT_ID.len()
                    + receiver_id.len()
                    + ACCOUNT_DATA_SEPARATOR.len()
                    + data_id.as_ref().len()
            }
            DBKey::PendingDataCount { receiver_id, receipt_id } => {
                col::PENDING_DATA_COUNT.len()
                    + receiver_id.len()
                    + ACCOUNT_DATA_SEPARATOR.len()
                    + receipt_id.as_ref().len()
            }
            DBKey::PostponedReceipt { receiver_id, receipt_id } => {
                col::POSTPONED_RECEIPT.len()
                    + receiver_id.len()
                    + ACCOUNT_DATA_SEPARATOR.len()
                    + receipt_id.as_ref().len()
            }
            DBKey::DelayedReceiptIndices => col::DELAYED_RECEIPT_INDICES.len(),
            DBKey::DelayedReceipt { .. } => col::DELAYED_RECEIPT.len() + size_of::<u64>(),
            DBKey::ContractData { account_id, key } => {
    ...
    pub fn to_vec(&self) -> Vec<u8> {
        ...
        match self {
            ...
            DBKey::DelayedReceiptIndices => {
                res.extend(col::DELAYED_RECEIPT_INDICES);
            }
            DBKey::DelayedReceipt { index } => {
                res.extend(col::DELAYED_RECEIPT_INDICES);
...
//                 "raw key is too short for DBKey::AccessKey",
//             ));
//         }
...
//             return Err(std::io::Error::new(
//                 std::io::ErrorKind::InvalidData,
//                 "raw key is too short for DBKey::ContractData",
//             ));
//         }
...
//                     std::io::ErrorKind::InvalidData,
//                     format!(
//                         "raw key AccountId has invalid UTF-8 format to be DBKey::{}",
//                         trie_key
//                     ),
//                 )
//             })?
...
//                 std::io::Error::new(
//                     std::io::ErrorKind::InvalidData,
//                     format!("raw key does not have a valid AccountId to be DBKey::{}", trie_key),
//                 )
//             })
//     }
...
//             return Err(std::io::Error::new(
//                 std::io::ErrorKind::InvalidData,
//                 "raw key does not have ACCOUNT_DATA_SEPARATOR to be DBKey::ContractData",
//             ));
//         };
...
//             return Err(std::io::Error::new(
//                 std::io::ErrorKind::InvalidData,
//                 "raw key does not have public key to be DBKey::AccessKey",
//             ));
//         };
...
//         Ok(DBKey::AccessKey { account_id, public_key })
//     }
...
//             return Err(std::io::Error::new(
//                 std::io::ErrorKind::InvalidData,
//                 format!("raw key does not have ACCOUNT_DATA_SEPARATOR to be DBKey::{}", col_name),
//             ));
//         };
//         let account_id = &account_id_prefix[..account_data_separator_position];
...
//             return Err(std::io::Error::new(
//                 std::io::ErrorKind::InvalidData,
//                 "raw key is too short for DBKey::ReceivedData",
//             ));
//         }
//         CryptoHash::try_from(&raw_key[prefix_len..]).map_err(|_| {
//             std::io::Error::new(
//                 std::io::ErrorKind::InvalidData,
//                 "Can't parse CryptoHash for DBKey::ReceivedData",
//             )
//         })
//     }
...
//             let key = DBKey::Account { account_id: account_id.clone() };
//             let raw_key = key.to_vec();
//             assert_eq!(raw_key.len(), key.len());
//             assert_eq!(
//                 db_key_parsers::parse_account_id_from_account_key(&raw_key).unwrap(),
//                 account_id
//             );
//             assert_eq!(
//                 db_key_parsers::parse_account_id_from_raw_key(&raw_key).unwrap().unwrap(),
//                 account_id
//             );
//         }
//     }
...
//             assert_eq!(raw_key.len(), key.len());
//             assert_eq!(
//                 db_key_parsers::parse_trie_key_access_key_from_raw_key(&raw_key).unwrap(),
//                 key
//             );
//             assert_eq!(
//                 db_key_parsers::parse_account_id_from_access_key_key(&raw_key).unwrap(),
//                 account_id
//             );
//             assert_eq!(
//                 db_key_parsers::parse_public_key_from_access_key_key(&raw_key, &account_id)
//                     .unwrap(),
//                 public_key
//             );
//             assert_eq!(
...
//         for account_id in OK_ACCOUNT_IDS.iter().map(|x| x.parse::<AccountId>().unwrap()) {
//             let key =
//                 DBKey::ContractData { account_id: account_id.clone(), key: data_key.to_vec() };
//             let raw_key = key.to_vec();
//             assert_eq!(raw_key.len(), key.len());
//             assert_eq!(
//                 db_key_parsers::parse_account_id_from_contract_data_key(&raw_key).unwrap(),
//                 account_id
//             );
//             assert_eq!(
//                 db_key_parsers::parse_data_key_from_contract_data_key(&raw_key, &account_id)
//                     .unwrap(),
//                 data_key
//             );
//             assert_eq!(
...
//         for account_id in OK_ACCOUNT_IDS.iter().map(|x| x.parse::<AccountId>().unwrap()) {
//             let key = DBKey::ContractCode { account_id: account_id.clone() };
//             let raw_key = key.to_vec();
//             assert_eq!(raw_key.len(), key.len());
//             assert_eq!(
//                 db_key_parsers::parse_account_id_from_contract_code_key(&raw_key).unwrap(),
//                 account_id
//             );
//             assert_eq!(
//                 db_key_parsers::parse_account_id_from_raw_key(&raw_key).unwrap().unwrap(),
...
//             assert_eq!(raw_key.len(), key.len());
//             assert_eq!(
//                 db_key_parsers::parse_account_id_from_received_data_key(&raw_key).unwrap(),
//                 account_id
//             );
//             assert_eq!(
//                 db_key_parsers::parse_account_id_from_raw_key(&raw_key).unwrap().unwrap(),
...
//             );
//             assert_eq!(
//                 db_key_parsers::parse_data_id_from_received_data_key(&raw_key, &account_id)
//                     .unwrap(),
//                 CryptoHash::default(),
//             );
//         }
...
//         for account_id in OK_ACCOUNT_IDS.iter().map(|x| x.parse::<AccountId>().unwrap()) {
//             let key = DBKey::PostponedReceipt {
//                 receiver_id: account_id.clone(),
...
//         for account_id in OK_ACCOUNT_IDS.iter().map(|x| x.parse::<AccountId>().unwrap()) {
//             let key = DBKey::PostponedReceiptId {
//                 receiver_id: account_id.clone(),
//                 data_id: CryptoHash::default(),
//             };
//             let raw_key = key.to_vec();
...
//         for account_id in OK_ACCOUNT_IDS.iter().map(|x| x.parse::<AccountId>().unwrap()) {
//             let key = DBKey::PendingDataCount {
//                 receiver_id: account_id.clone(),
//                 receipt_id: CryptoHash::default(),
//             };
//             let raw_key = key.to_vec();
...
//         let key = DBKey::DelayedReceiptIndices;
//         let raw_key = key.to_vec();
//         assert!(db_key_parsers::parse_account_id_from_raw_key(&raw_key).unwrap().is_none());
//         let key = DBKey::DelayedReceipt { index: 0 };
//         let raw_key = key.to_vec();
//         assert!(db_key_parsers::parse_account_id_from_raw_key(&raw_key).unwrap().is_none());
//     }
// }
