...
pub mod driver {
    use file::FileHandle;
    use metadata::EncryptionSchema;
    use crate::types::{
        crypto::{SecretboxKey, BoxSecretKey},
        ipfs::{CID},
        file::{DEFAULT_CHUNK_SIZE, Hash},
    ...
    pub fn pre_upstream(file: &FileHandle) -> Result<(SecretboxKey ,Chunks), ProtocolError> {
        ...
        let mut buf = [u8; DEFAULT_CHUNK_SIZE];
        let mut hash: Option<Hash> = None;
        let mut chunks: Chunks = Vec::new();

        loop {
            // read current chunk
            let len = file.read_exact(buf).map_err(file_error)?;
            if len == 0 {
                break;
            }

            hash = match hash {
                None => FileHandle::sha256_checksum(buf)
                Some(h) => {
            ...
            let mut chunk = file.deflate_chunk(&buf);
            chunk = crypto::NaClSecretBox::encrypt(&sealing_key, &chunk).map_err(crypto_error)?;
            chunks.push(chunk);
        };
        (sealing_key, chunks)
    }
    ...
    ) -> Result<SealedMetadata, ProtocolError> {
    ...
    pub fn pre_downstream(
        ...
        let pre_seal = sealed.unseal(keys).map_err(metadata_error)?;
    ...
    pub fn post_downstream(
        ...
        for chunk in chunks {
            let mut buf = crypto::NaClSecretBox::decrypt(&sealing_key, &chunk).map_err(crypto_error)?;
            buf = file.inflate_chunk(&buf).map_err(file_error)?;
