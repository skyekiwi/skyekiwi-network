...
use wasmi::{
    ...
    memory_units::{Pages, Bytes, size_of}, TrapKind,
};
...
fn map_invoke_err(error: wasmi::Error) -> VMError {
    // Real Trap(Host(HostError(GasExceeded)))
    let result = match error {
        wasmi::Error::Trap(e) => {
            match e.kind() {
                TrapKind::Unreachable => VMError::FunctionCallError(FunctionCallError::WasmTrap(WasmTrap::Unreachable)),
                TrapKind::MemoryAccessOutOfBounds => VMError::FunctionCallError(FunctionCallError::WasmTrap(WasmTrap::MemoryOutOfBounds)),
                TrapKind::TableAccessOutOfBounds => VMError::FunctionCallError(FunctionCallError::WasmTrap(WasmTrap::MemoryOutOfBounds)),
                TrapKind::ElemUninitialized => VMError::FunctionCallError(FunctionCallError::WasmTrap(WasmTrap::IndirectCallToNull)),
                TrapKind::DivisionByZero => VMError::FunctionCallError(FunctionCallError::WasmTrap(WasmTrap::IllegalArithmetic)),
                TrapKind::IntegerOverflow => VMError::FunctionCallError(FunctionCallError::WasmTrap(WasmTrap::IllegalArithmetic)),
                TrapKind::InvalidConversionToInt => VMError::FunctionCallError(FunctionCallError::WasmTrap(WasmTrap::IllegalArithmetic)),
                TrapKind::StackOverflow => VMError::FunctionCallError(FunctionCallError::WasmTrap(WasmTrap::StackOverflow)),
                TrapKind::UnexpectedSignature => VMError::FunctionCallError(FunctionCallError::WasmTrap(WasmTrap::IncorrectCallIndirectSignature)),
                TrapKind::Host(err) => VMError::FunctionCallError(FunctionCallError::HostError(err.downcast_ref::<HostError>().unwrap().clone())),
            }
        },
        // TODO: this is not right ... leave it here for now
        _ => VMError::FunctionCallError(FunctionCallError::WasmTrap(WasmTrap::IncorrectCallIndirectSignature)),
    };
    result
}
