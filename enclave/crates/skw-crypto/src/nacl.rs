...
impl NaClBox {
	...
	pub fn keypair_from_seed(secret_key: BoxSecretKey) -> BoxKeyPair {
		...
		BoxKeyPair { public_key, secret_key }
	}
	...
	pub fn encrypt(
		...
		let ephemeral_key = sgx_rand::random::<[u8; BOX_SECRET_KEY_LEN]>();
		let public_key = PublicKey::from(&StaticSecret::from(ephemeral_key)).to_bytes();

		let ecdh = StaticSecret::from(ephemeral_key);
		...
		if let Ok((nonce, cipher_text)) = NaClSecretBox::encrypt(shared_secret.as_bytes(), msg) {
			Ok((public_key, nonce, cipher_text))
		} else {
